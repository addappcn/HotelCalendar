...
public class MonthView extends LinearLayout {
  ...
  public void init(MonthDescriptor month, List<List<MonthCellDescriptor>> cells,
    ...
    calendar.setYear(month.getYear());
    calendar.setMonth(month.getMonth()+1);
//    SimpleDateFormat FORMATTER_WITH_DD = new SimpleDateFormat("dd", Locale.CHINA);
//    int day = Integer.parseInt(FORMATTER_WITH_DD.format(month.getDate()));
//    calendar.setDay(day);
//    LunarCalendar.setupLunarCalendar(calendar);
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if (i < numRows) {
        ...
        for (int c = 0; c < week.size(); c++) {
          ...
          calendar.setDay(cell.getValue());
          LunarCalendar.setupLunarCalendar(calendar);
          String lunar = LunarCalendar.getLunarText(calendar);//节日和农历
          //设置天的数据
          if (!cellView.getDayOfMonthTextView().getText().equals(cellDate)) {
            if(cell.getRangeState() == RangeState.FIRST){
              cellView.getDayOfMonthTextView().setText(getDealDayStr(lunar,cellDate,"入住"));
            }else if(cell.getRangeState() == RangeState.LAST){
              cellView.getDayOfMonthTextView().setText(getDealDayStr(lunar,cellDate,"离店"));
            }else{
              cellView.getDayOfMonthTextView().setText(getDealDayStr(lunar,cellDate,""));
            }
          }
          //选中处理
          if(cell.isSelected() && cell.getRangeState()==RangeState.NONE){
            cellView.getDayOfMonthTextView().setText(getDealDayStr(lunar,cellDate,"入住"));
          }
  ...
  private CharSequence getDealDayStr(String upStr,String dayStr,String downStr){
    SpanUtils spanUtils = new SpanUtils();
     spanUtils.append(upStr)
              .setFontSize(10,true)
              .appendLine()
              .append(dayStr)
              .appendLine()
